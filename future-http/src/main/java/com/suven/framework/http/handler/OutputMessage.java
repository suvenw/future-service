...
public class OutputMessage  extends BaseHttpMessageHandlerConverter {
    ...
    public void writeResult(IResponseResult iResponseResult, Object responseData, String... errParam)  {
        this.writeResponseData(iResponseResult,responseData,errParam);
        this.writeStream(iResponseResult);
        if(iResponseResult.code() == SysResultCodeEnum.SYS_SUCCESS.getCode()){
            this.printSuccessLog(logger);
        }else {
            this.printErrorLogForRequestMessage(logger,iResponseResult.code(),iResponseResult.message());
        }
    }
    /**
     * 写入客户端结果/消息。
