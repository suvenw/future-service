    ...
    methods:{
      ...
      loadList (arg){
        ...
        getAction(this.url.list,params).then((res)=>{
          ...
          this.dataSource1 = res.data.data1;
          this.dataSource2 = res.data.data2;
        ...
        getAction(this.url.queryDiskInfo).then((res)=>{
          if(res.code ==0){
            ...
            for(let a in res.data){
              let tempNum = Number(res.data[a].max)
              if(tempNum>one){
                one = tempNum
                two = res.data[a].name
              }
            }
            let ontItem = res.data.filter((item)=>{return item.name == two})[0]
            ...
            if(res.data.length>1){
              let one2=0,two2="";
              for(let a in res.data){
                if(res.data[a].name == two){
                  continue;
                }
                let tempNum = Number(res.data[a].max)
                if(tempNum>one2){
                  one2 = tempNum
                  two2 = res.data[a].name
                }
              }
              let one2Item = res.data.filter((item)=>{return item.name == two2})[0]
