  ...
  export default {
    ...
    data() {
      return {
        ...
        url: {
          ...
          deleteBatch: "/sys/user/deleteUserInDepart",
        },
        ipagination: {
          pageSize: 10
        }
    ...
    methods: {
      ...
      batchDel: function () {
        ...
        if (this.selectedRowKeys.length <= 0) {
          ...
          this.$confirm({
            ...
            content: "是否删除选中数据?",
            onOk: function () {
              deleteAction(that.url.deleteBatch, {depId: that.currentDeptId, idList: ids}).then((res) => {
                if (res.code ==0) {
      ...
      handleDelete: function (id) {
        ...
        var that = this;
        deleteAction(that.url.delete, {depId: this.currentDeptId, idList: id}).then((res) => {
          if (res.code ==0) {
            that.$message.success(res.msg);
            if (this.selectedRowKeys.length>0){
      ...
      selectOK(data) {
        ...
        if (ids == '') {
          this.$message.error("最少要选择一个用户!")
          return
        } 
        let params = new FormData()
        params.append('depId',this.currentDeptId)
