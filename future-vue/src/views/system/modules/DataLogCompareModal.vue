    ...
    methods: {
      loadData(dataId1, dataId2) {
        ...
        getAction(that.url.queryCompareUrl, {dataId1: dataId1, dataId2: dataId2}).then((res) => {
          if (res.code ==0) {
            that.dataVersion1Num = res.data[0].dataVersion;
            that.dataVersion2Num = res.data[1].dataVersion;
            let json1 = JSON.parse(res.data[0].dataContent);
            let json2 = JSON.parse(res.data[1].dataContent);
