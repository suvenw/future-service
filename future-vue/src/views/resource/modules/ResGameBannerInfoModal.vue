    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="广告位置" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select v-decorator="[ 'type', validatorRules.type]" placeholder="请选择广告位置" @change="onChangeType">
            <a-select-option :value="1">顶部</a-select-option>
            <a-select-option :value="2">中部</a-select-option>
            <a-select-option :value="3">底部</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          ...
          label="广告名称">
          <a-input placeholder="请输入广告名称" v-decorator="['name', validatorRules.name]"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="链接地址">
          <a-input placeholder="请输入链接地址" v-decorator="['link', validatorRules.link]"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="排序序号">
          <a-input-number placeholder="请输入排序序号" v-decorator="['sort', validatorRules.sort]"/>
        </a-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            imageBeforeUpload(file) {
                ...
                if (_this.type === '' || _this.type === undefined) {
                ...
                return new Promise((resolve, reject) => {
                  this.imageRead(file).then((img) => {
                    if (_this.type === 1 && img.width != 1077 && img.height != 606) {
                        this.$message.error('顶部广告图片尺寸大小为1077 × 606像素!')
                        reject(file);
                    } else if (_this.type === 2 && img.width != 1077 && img.height != 264) {
                        this.$message.error('中部广告图片尺寸大小为1077 × 264像素!')
                        reject(file);
                    }  else if (_this.type === 3 && img.width != 351 && img.height != 480) {
                        this.$message.error('底部广告图片尺寸大小为351 × 480像素!')
                        reject(file);
                    } else {
                        resolve(file);
                    }
            ...
            onChangeDate(date) {
                ...
                if (date[0] != undefined && date[1] != undefined) {
                    this.model.startDate = moment(date[0], 'YYYY-MM-DD HH:mm:ss').valueOf()
                    this.model.endDate = moment(date[1], 'YYYY-MM-DD HH:mm:ss').valueOf()
                }
            },
